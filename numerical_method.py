...
I_stc = 1
T_ref = 25
alpha_coefficient = 0.004  # PLACEHOLDER
reference_efficiency = 0.2  # PLACEHOLDER
...
def solar_panel_efficiency(temperature_arr):
    return reference_efficiency * (1 - alpha_coefficient * (temperature_arr - T_ref))
...
def improved_euler_method(ext_temp, rad_t_low, rad_t_high, duration, time_step, date, plot_cond=False):
    duration_seconds = duration * 3600
    time_array = np.arange(0, duration_seconds, time_step)
    temperature_array = np.zeros_like(time_array)
    temperature_array[0] = ext_temp
    ...
    for i in range(1, len(time_array)):
        # First slope estimate (Euler step)
        dTdt1, heater_on_1 = heating_cooling_eqn(
            ext_temp, (rad_t_low, rad_t_high), temperature_array[i-1], heater_on)

        T_predict = temperature_array[i-1] + time_step * dTdt1

        # Second slope estimate using predicted T
        dTdt2, heater_on_2 = heating_cooling_eqn(
            ext_temp, (rad_t_low, rad_t_high), T_predict, heater_on)

        # Average the two derivatives
        dTdt_avg = 0.5 * (dTdt1 + dTdt2)

        # Update the temperature
        temperature_array[i] = temperature_array[i-1] + time_step * dTdt_avg

        # Use average heating status to estimate energy (can tweak this logic if needed)
        heater_status_avg = heater_on_1 or heater_on_2
        if heater_status_avg:
            total_energy += time_step * Q_max

        # Optional: update heater_on state for the next iteration
        heater_on = heater_on_2
